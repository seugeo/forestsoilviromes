...
```
# contigs found in only burned forest
```{r}
burned.otu <- otu[, blodgett_data$Disturbance=="Yes_Burn"]
burned.otu <- burned.otu[rowSums(burned.otu != 0) > 0, ]
burned_contigs <- rownames(burned.otu)
other.otu <- otu[, blodgett_data$Disturbance!="Yes_Burn"]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
burned_contigs <- setdiff(burned_contigs, other_contigs) # 29,196 contigs unique to burned forest samples
```
# contigs found in only unburned forest
```{r}
unburned.otu <- otu[, blodgett_data$Disturbance=="No"]
unburned.otu <- unburned.otu[rowSums(unburned.otu != 0) > 0, ]
unburned_contigs <- rownames(unburned.otu)
other.otu <- otu[, blodgett_data$Disturbance!="No"]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
unburned_contigs <- setdiff(unburned_contigs, other_contigs) # 132,918 contigs unique to unburned forest samples
```
# contigs found in only heated forest
```{r}
ninety.otu <- otu[, blodgett_data$Disturbance=="Yes_90ºC"]
ninety.otu <- ninety.otu[rowSums(ninety.otu != 0) > 0, ]
ninety_contigs <- rownames(ninety.otu)
other.otu <- otu[, blodgett_data$Disturbance!="Yes_90ºC"]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
ninety_contigs <- setdiff(ninety_contigs, other_contigs) # 77 contigs unique to 90C forest samples

sixty.otu <- otu[, blodgett_data$Disturbance=="Yes_60ºC"]
sixty.otu <- sixty.otu[rowSums(sixty.otu != 0) > 0, ]
sixty_contigs <- rownames(sixty.otu)
other.otu <- otu[, blodgett_data$Disturbance!="Yes_60ºC"]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
sixty_contigs <- setdiff(sixty_contigs, other_contigs) # 420 contigs unique to 60C forest samples

thirty.otu <- otu[, blodgett_data$Disturbance=="Yes_30ºC"]
thirty.otu <- thirty.otu[rowSums(thirty.otu != 0) > 0, ]
thirty_contigs <- rownames(thirty.otu)
other.otu <- otu[, blodgett_data$Disturbance!="Yes_30ºC"]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
thirty_contigs <- setdiff(thirty_contigs, other_contigs) # 128 contigs unique to 30C forest samples

heated.otu <- otu[, blodgett_data$Disturbance %in% c("Yes_30ºC", "Yes_60ºC", "Yes_90ºC")]
heated.otu <- heated.otu[rowSums(heated.otu != 0) > 0, ]
heated_contigs <- rownames(heated.otu)
other.otu <- otu[, !blodgett_data$Disturbance %in% c("Yes_30ºC", "Yes_60ºC", "Yes_90ºC")]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
heated_contigs <- setdiff(heated_contigs, other_contigs) 
heated_contigs <- setdiff(heated_contigs, thirty_contigs)
heated_contigs <- setdiff(heated_contigs, sixty_contigs)
heated_contigs <- setdiff(heated_contigs, ninety_contigs) # 153 contigs unique to heated forest samples and found in multiple temperatures
```
# contigs found in burned and heated forest
```{r}
disturbed.otu <- otu[, blodgett_data$Disturbance %in% c("Yes_30ºC", "Yes_60ºC", "Yes_90ºC", "Yes_Burn")]
disturbed.otu <- disturbed.otu[rowSums(disturbed.otu != 0) > 0, ]
disturbed_contigs <- rownames(disturbed.otu)
other.otu <- otu[, !blodgett_data$Disturbance %in% c("Yes_30ºC", "Yes_60ºC", "Yes_90ºC", "Yes_Burn")]
other.otu <- other.otu[rowSums(other.otu != 0) > 0, ]
other_contigs <- rownames(other.otu)
disturbed_contigs <- setdiff(disturbed_contigs, other_contigs) 
disturbed_contigs <- setdiff(disturbed_contigs, thirty_contigs)
disturbed_contigs <- setdiff(disturbed_contigs, sixty_contigs)
disturbed_contigs <- setdiff(disturbed_contigs, ninety_contigs)
disturbed_contigs <- setdiff(disturbed_contigs, burned_contigs)
disturbed_contigs <- setdiff(disturbed_contigs, heated_contigs)# 88 contigs unique to disturbed forest samples and found in multiple disturbed states

overlap.otu <- otu
overlap.otu <- overlap.otu[rowSums(disturbed.otu != 0) > 0, ]
overlap_contigs <- rownames(overlap.otu)
overlap_contigs <- setdiff(overlap_contigs, disturbed_contigs)
overlap_contigs <- setdiff(overlap_contigs, thirty_contigs)
overlap_contigs <- setdiff(overlap_contigs, sixty_contigs)
overlap_contigs <- setdiff(overlap_contigs, ninety_contigs)
overlap_contigs <- setdiff(overlap_contigs, heated_contigs)
overlap_contigs <- setdiff(overlap_contigs, burned_contigs)
overlap_contigs <- setdiff(overlap_contigs, unburned_contigs) # 109,037 contigs unique to blodgett forest samples and found in multiple disturbed and undisturbed states
```
# combining contigs to make dataframe
```{r}
ninety <- data.frame(vOTU = ninety_contigs) %>%
  mutate(Habitat = "Forest_Heated90ºC")
sixty <- data.frame(vOTU = sixty_contigs) %>%
  mutate(Habitat = "Forest_Heated60ºC")
thirty <- data.frame(vOTU = thirty_contigs) %>%
  mutate(Habitat = "Forest_Heated30ºC")
heated <- data.frame(vOTU = heated_contigs) %>%
  mutate(Habitat = "Forest_Heated")
burned <- data.frame(vOTU = burned_contigs) %>%
  mutate(Habitat = "Forest_Burned")
disturbed <- data.frame(vOTU = disturbed_contigs) %>%
  mutate(Habitat = "Forest_Disturbed")
unburned <- data.frame(vOTU = unburned_contigs) %>%
  mutate(Habitat = "Forest_Undisturbed")
forest <- data.frame(vOTU = overlap_contigs) %>%
  mutate(Habitat = "Forest")
blodgett <- rbind(ninety, sixty, thirty, heated, burned, disturbed, unburned, forest)
saveRDS(blodgett, "C:/Users/segeo/Box Sync/EmersonLab/Research/blodgett_contigs_habitat.RDS")
```



# DNA Yields
# DNA Yields without NAs
```{r}
dnayields <- readRDS("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/blodgett_dnayields.RDS")

DNAyield <- dnayields %>%
  ggplot(aes(x = factor(Date, level = burn_date_order), y = DNA_g, fill = factor(Depth, level = depth_order))) +
...
ggsave("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/BoxPlots/DNAyield/Blodgett_DNAyield.pdf", width = 200, height = 120, units = "mm", dpi = 500)
...
```
# DNA Yields with NA replaced with 0 ng/g soil
```{r}
...
```{r}
# this is separating out depth, which I don't think I care about anymore
# pre vs. post burn
na_yields_03_burn <- na_yields.stats %>%
  filter(Depth == "0-3cm"&Treatment == "Prescribed_Burn")
kruskal.test(DNA_g ~ Date, na_yields_03_burn)
wilcox <- pairwise_wilcox_test(
  data = na_yields_03_burn,
  formula = DNA_g ~ Date,
  p.adjust.method = "bonferroni")
na_yields_03_control <- na_yields.stats %>%
  filter(Depth == "0-3cm"&Treatment == "Prescribed_Control")
kruskal.test(DNA_g ~ Date, na_yields_03_control)
wilcox <- pairwise_wilcox_test(
  data = na_yields_03_control,
  formula = DNA_g ~ Date,
  p.adjust.method = "holm")
kruskal.test(DNA_g ~ Burn, na_yields_03_control)
na_yields_36_burn <- na_yields.stats %>%
  filter(Depth == "3-6cm"&Treatment == "Prescribed_Burn")
kruskal.test(DNA_g ~ Burn, na_yields_36_burn)
na_yields_36_control <- na_yields.stats %>%
  filter(Depth == "3-6cm"&Treatment == "Prescribed_Control")
kruskal.test(DNA_g ~ Burn, na_yields_36_control)
na_yields_burn <- na_yields.stats %>%
  filter(Treatment == "Prescribed_Burn")
kruskal.test(DNA_g ~ Date, na_yields_burn)
na_yields_control <- na_yields.stats %>%
...
na.49_yields_03_burn <- na.49_yields.stats %>%
  filter(Depth == "0-3cm"&Treatment == "Prescribed_Burn")
kruskal.test(DNA_g ~ Disturbance, na.49_yields_03_burn)
na.49_yields_03_control <- na.49_yields.stats %>%
  filter(Depth == "0-3cm"&Treatment == "Prescribed_Control")
kruskal.test(DNA_g ~ Burn, na.49_yields_03_control)

na.49_yields_36_burn <- na.49_yields.stats %>%
  filter(Depth == "3-6cm"&Treatment == "Prescribed_Burn")
kruskal.test(DNA_g ~ Burn, na.49_yields_36_burn)

na.49_yields_36_control <- na.49_yields.stats %>%
  filter(Depth == "3-6cm"&Treatment == "Prescribed_Control")
kruskal.test(DNA_g ~ Burn, na.49_yields_36_control)
```

# DNA Yields with NA replaced with 0.49 ng/g soil
```{r}
...
na.49_DNAyield <- na.49_dnayields %>%
  ggplot(aes(x = Date, y = DNA_g, fill = Treatment)) +
  annotate('rect', xmin = -Inf, xmax = 2.5, ymin = -Inf, ymax = Inf, fill = 'palegreen', alpha = 0.5) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(aes(color = Depth), position = position_jitter(), size = 1.5) +
...
na.49_DNAyield + theme(strip.text = element_text(size=16))

ggsave("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/BoxPlots/DNAyield/Blodgett_na.49_DNAyield.pdf", width = 230, height = 120, units = "mm", dpi = 500)
...
# The Subsets I use for figures
# Prescribed Burn (2021) (p)
```{r}
...
p.count <- otu.count[, blodgett_data$Treatment=="Prescribed_Burn" | blodgett_data$Treatment=="Prescribed_Control"] %>%
 rmv_sngl()
common_contigs <- intersect(rownames(p.otu), rownames(p.count))
p.count <- p.count[rownames(p.count) %in% common_contigs, ]

saveRDS(p.map, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/p.map.RDS")
...
# Prescribed Burn (2021) - T3 + burn samples only
T3d.otu <- otu[, blodgett_data$Date=="4/27/2021" & blodgett_data$Disturbance=="Yes_Burn"] %>%
  rmv_sngl()
T3d.otu.rel <- relativize(T3d.otu)
colSums(T3d.otu.rel)
T3d.map <- subset(blodgett_data, Date=="4/27/2021" & blodgett_data$Disturbance=="Yes_Burn")
T3d.count <- otu.count[, blodgett_data$Date=="4/27/2021" & blodgett_data$Disturbance=="Yes_Burn"] %>%
  rmv_sngl()
common_contigs <- intersect(rownames(T3d.otu), rownames(T3d.count))
T3d.count <- T3d.count[rownames(T3d.count) %in% common_contigs, ]
...
# Prescribed Burn (2021) - T4 + burn samples only
T4d.otu <- otu[, blodgett_data$Date=="5/8/2021" & blodgett_data$Disturbance=="Yes_Burn"] %>%
  rmv_sngl()
T4d.otu.rel <- relativize(T4d.otu)
colSums(T4d.otu.rel)
T4d.map <- subset(blodgett_data, Date=="5/8/2021" & blodgett_data$Disturbance=="Yes_Burn")
T4d.count <- otu.count[, blodgett_data$Date=="5/8/2021" & blodgett_data$Disturbance=="Yes_Burn"] %>%
  rmv_sngl()
common_contigs <- intersect(rownames(T4d.otu), rownames(T4d.count))
T4d.count <- T4d.count[rownames(T4d.count) %in% common_contigs, ]
...
```
# Treatment Condition / Plots / Depths
```{r}
# 2021 (only control)
...
pcoa.plot.plot.disturbance(p.otu.map, x_col = pcoa1, y_col = pcoa2, color_col= Plot, shape_col = Disturbance, x_label = "PCo1 (5.34% variance explained)", y_label = "PCo2 (4.32% variance explained)", title = "2021 Blodgett Viromes", filename = "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/PCoA/viromes/2021_blodgett_plot_disturbance.pdf")


# run PERMANOVAs
variables <- c("Disturbance", "Depth", "Date", "Treatment", "Plot")
adonis2(p.otu.dist ~ p.otu.map$'Disturbance' + p.otu.map$'Plot' + p.otu.map$'Depth' + p.otu.map$'Date', by = "margin")
adonis2(p.otu.dist ~ p.otu.map$'Disturbance' + p.otu.map$'Plot' + p.otu.map$'Depth' + p.otu.map$'Date')
adonis2(p.otu.dist ~ p.otu.map$'Plot' + p.otu.map$'Date' + p.otu.map$'Depth' + p.otu.map$'Disturbance')
adonis2(p.otu.dist ~ p.otu.map$'Disturbance' + p.otu.map$'Depth' + p.otu.map$'Date' + p.otu.map$'Plot')


adonis(p.otu.dist ~ p.otu.map$'Plot')
...
pcoa.sub.plot.disturbance(pxB1xU1.otu.map, x_col = pcoa1, y_col = pcoa2, color_col= Plot, shape_col = Disturbance, x_label = "PCo1 (5.90% variance explained)", y_label = "PCo2 (5.56% variance explained)", title = "2021 Blodgett Viromes xB1xU1", filename = "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/PCoA/viromes/2021_blodgett_plot_disturbance_xB1_xU1.pdf")
# run PERMANOVAs

### I DON'T KNOW WHAT THE BEST ORDER OF VARIABLES I SHOULD DO
adonis2(pxB1xU1.otu.dist ~ pxB1xU1.otu.map$'Disturbance' + pxB1xU1.otu.map$'Plot' + pxB1xU1.otu.map$'Depth' + pxB1xU1.otu.map$'Date', by = "margin")
...
```
# Disturbed Samples Only - PC1 gradient
```{r}
...
ggsave("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/PCoA/viromes/DisturbedxB1_PCoA_PC1.pdf", width = 180, height = 125, units = "mm", dpi = 500)
...
# T3
T3.otu.dist <- distance(T3.otu.rel)
T3.otu.dis.matrix <- as.matrix(T3.otu.dist)
saveRDS(T3.otu.dis.matrix, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/T3.otu.dis.matrix.RDS")

# T4
T4.otu.dist <- distance(T4.otu.rel)
T4.otu.dis.matrix <- as.matrix(T4.otu.dist)
saveRDS(T4.otu.dis.matrix, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/T4.otu.dis.matrix.RDS")
...
# Pairwise difference in burn severity in soil profile
```{r}
d_pc1
d.metadata
...
```{r}
Richness <- p.map %>%
  ggplot(aes(x = Timepoint, y = Richness, fill = Treatment)) +
  annotate('rect', xmin = -Inf, xmax = 2.5, ymin = -Inf, ymax = Inf, fill = 'palegreen', alpha = 0.5) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(aes(color = Depth), position = position_jitter(), size = 1.5) +
  xlab("Time") +
...
ggsave("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/BoxPlots/Richness/All_Richness.pdf", width = 200, height = 120, units = "mm", dpi = 500)
```

# Richness along burn severity
```{r}
...
intrxn <- lm(Richness ~ Comp.1 * Depth, data = d.map.pc1)
summary(intrxn)
...
saveRDS(p.otu.phylum.map, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/p.otu.phylum.map.rds")
...
glm_missing <- glm( Missing ~ Comp.1, data = d_pc1, family = binomial )
summary(glm_missing)
...
saveRDS(d.otu.phylum.map, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/d.otu.phylum.map.rds")
...
```{r}
#Stats
d.host.stats <- d.otu.phylum.map

acido_d <- d.host.stats %>%
  filter(HostPhylum == "Acidobacteriota")
ggplot(acido_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Acidobacteriota Host") +
  theme_minimal()
cor.test(acido_d$Comp.1, acido_d$RelAbd, method = "spearman")

actino_d <- d.host.stats %>%
  filter(HostPhylum == "Actinobacteriota")
ggplot(actino_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Actinobacteriota Host") +
  theme_minimal()
cor.test(actino_d$Comp.1, actino_d$RelAbd, method = "spearman")

bact_d <- d.host.stats %>%
  filter(HostPhylum == "Bacteroidota")
ggplot(bact_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Bacteroidota Host") +
  theme_minimal()
cor.test(bact_d$Comp.1, bact_d$RelAbd, method = "spearman")

chloro_d <- d.host.stats %>%
  filter(HostPhylum == "Chloroflexota")
ggplot(chloro_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Chloroflexota Host") +
  theme_minimal()
cor.test(chloro_d$Comp.1, chloro_d$RelAbd, method = "spearman")

cyano_d <- d.host.stats %>%
  filter(HostPhylum == "Cyanobacteria")
ggplot(cyano_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Cyanobacteria Host") +
  theme_minimal()
cor.test(cyano_d$Comp.1, cyano_d$RelAbd, method = "spearman")

firm_d <- d.host.stats %>%
  filter(HostPhylum == "Firmicutes")
ggplot(firm_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Firmicutes Host") +
  theme_minimal()
cor.test(firm_d$Comp.1, firm_d$RelAbd, method = "spearman")

myxo_d <- d.host.stats %>%
  filter(HostPhylum == "Myxococcota")
ggplot(myxo_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Myxococcota Host") +
  theme_minimal()
cor.test(myxo_d$Comp.1, myxo_d$RelAbd, method = "spearman")

nitro_d <- d.host.stats %>%
  filter(HostPhylum == "Nitrospirota")
ggplot(nitro_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Nitrospirota Host") +
  theme_minimal()
cor.test(nitro_d$Comp.1, nitro_d$RelAbd, method = "spearman")

proteo_d <- d.host.stats %>%
  filter(HostPhylum == "Proteobacteria")
ggplot(proteo_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Proteobacteria Host") +
  theme_minimal()
cor.test(proteo_d$Comp.1, proteo_d$RelAbd, method = "spearman")

verru_d <- d.host.stats %>%
  filter(HostPhylum == "Verrucomicrobiota")
ggplot(verru_d, aes(x = Comp.1, y = RelAbd)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Verrucomicrobiota Host") +
  theme_minimal()
cor.test(verru_d$Comp.1, verru_d$RelAbd, method = "spearman")

unknown_d <- d.otu.unknown.phylum %>%
  filter(HostPhylum == "Unknown")
unknown_d$Detected <- 1 - unknown_d$RelAbd
ggplot(unknown_d, aes(x = Comp.1, y = Detected)) +
  geom_point() +
  labs(title = "Scatter Plot",
       x = "Burn Severity",
       y = "Relative Abundance of Detected Host") +
  theme_minimal()
cor.test(unknown_d$Comp.1, unknown_d$Detected, method = "spearman")


average_detection <- mean(unknown_d$Detected, na.rm = TRUE)

glm_missing <- glm( missing ~ PC1, data = df, family = binomial )
summary(glm_missing)

# Where the column, ‘missing’, in the data.frame ‘df’ can be 0 or 1, or a factor. If you make ‘1’  = Sample is missing, then the coefficient for PC1 (the slope coefficient) will tell you how much an increase in the odds of missingness for 1 unit increase in the PC1 score. 
...
# subset with just one time point at a time (remove time component)
...
# T3, burn samples
T3d.pc1 <- semi_join(d_pc1, T3d.map, by = "ID")
T3d.otu.da <- T3d.count
T3d.otu.da <- rownames_to_column(T3d.otu.da, var = "vOTU")
T3d.otu.da <- column_to_rownames(T3d.otu.da, var = "vOTU")
total_counts <- rowSums(T3d.otu.da)
top_contigs <- names(sort(total_counts, decreasing = TRUE))[1:5000]
filtered_T3d.otu.da <- T3d.otu.da[top_contigs, ]

#### THIS DOESN'T FILTER THE TABLE ENOUGH ... what this is doing now is making sure there is at least 1 count of
#### a vOTU in both replicates of a subplot at a timepoint
#### I think I need to do something more stringent, I think the vOTUs need to appear in more than one subgroup...
T3_long <- T3d.otu.da %>%
  pivot_longer(cols=2:16, names_to = "ID", values_to = "Count")

subgroup <- blodgett_data %>%
  select(ID, SubPlot, Date)

T3_long <- left_join(T3_long, subgroup, by="ID")
filter_contigs <- function(data) {
  data %>%
    group_by(vOTU, SubPlot) %>%
    filter(all(Count >= 100)) %>%  # Ensure count is at least 10 in all replicates of each subplot
    ungroup() %>%
    group_by(vOTU) %>%
    filter(n_distinct(SubPlot) > 2) %>%  # Ensure the condition is met in more than 2 subplots
    ungroup()
}
T3_filtered <- filter_contigs(T3_long)
filtered_abundance_wide <- T3_filtered %>%
  pivot_wider(names_from = ID, values_from = Count) %>%
  select(vOTU, all_of(unique(T3d.map$ID)))
summarized_data <- filtered_abundance_wide %>%
  group_by(vOTU) %>%
  summarize_all(sum, na.rm = TRUE)
summarized_data <- column_to_rownames(summarized_data, var = "vOTU")
### But I don't really have replicates...each sample is distinct...so this doesn't really work...




#make sure Comp.1 is numeric
T3d.pc1 <- as.data.frame(T3d.pc1)
T3d.pc1 <- col_to_row(T3d.pc1, "ID", "ID")

set.seed(1)
T3d_da_analysis <- differentialTest(formula = ~ Comp.1,
                                phi.formula = ~ Comp.1,
                                formula_null = ~ 1,
                                phi.formula_null = ~ Comp.1,
                                test = "Wald", boot = FALSE,
                                data = filtered_T3d.otu.da, 
                                sample_data = T3d.pc1, 
                                taxa_are_rows = TRUE, 
                                fdr_cutoff = 0.05)
T3d_da_analysis
significant_taxa <- T3d_da_analysis$significant_taxa
taxa <- plot(T3d_da_analysis, data_only = TRUE)



ggsave("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/corncob/T3.pdf", width = 250, height = 1250, units = "mm", dpi = 500)

# T4, burn samples
T4d.pc1 <- semi_join(d_pc1, T4d.map, by = "ID")
T4d.otu.da <- T4d.count
T4d.otu.da <- T4d.otu.da[rowSums(T4d.otu.da>1) >11,]
T4d.pc1 <- as.data.frame(T4d.pc1)
T4d.pc1 <- col_to_row(T4d.pc1, "ID", "ID")

set.seed(1)
T4d_da_analysis <- differentialTest(formula = ~ Comp.1,
                                phi.formula = ~ Comp.1,
                                formula_null = ~ 1,
                                phi.formula_null = ~ Comp.1,
                                test = "Wald", boot = FALSE,
                                ...
                                fdr_cutoff = 0.05)
T4d_da_analysis
T4d_da_analysis$significant_taxa
taxa <- plot(T4d_da_analysis)
...
```{r}
contig_list <- as.data.frame(taxa$taxa)
selected_contigs <- T3d.otu.rel[rownames(T3d.otu.rel) %in% contig_list$`taxa$taxa`, ]
abundance_long <- selected_contigs %>%
  rownames_to_column("vOTU") %>%
  gather(key = "ID", value = "RelAbd", -vOTU)
merged_data <- merge(abundance_long, T3d.pc1, by.x = "ID", by.y = "ID")
ordered_data <- merged_data %>%
  arrange(Comp.1)

ggplot(merged_data, aes(x = Comp.1, y = RelAbd, group = vOTU, color = vOTU)) +
  geom_line(alpha = 0.3) +
  labs(title = "Change in Abundance Along Continuous Covariate",
       x = "Covariate",
       y = "Abundance") +
  theme_minimal() +
