...
na_yields_03_burn <- na_yields.stats %>%
  filter(Depth == "0-3cm"&Treatment == "Prescribed_Burn")
kruskal.test(DNA_g ~ Disturbance, na_yields_03_burn)
...
#### I think I need to do something more stringent, probably with a minimum count of... 10?

filter_contigs <- function(data) {
  data %>%
    group_by(vOTU, SubPlot, Date) %>%
    filter(all(Count > 20)) %>%
    ungroup()
}
...
T3d_da_analysis <- differentialTest(formula = ~ Comp.1,
                                ...
                                fdr_cutoff = 0.05)
T3d_da_analysis
T3d_da_analysis$significant_taxa
plot(T3d_da_analysis)
