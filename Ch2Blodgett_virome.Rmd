...
na_DNAyield + theme(strip.text = element_text(size=14))
...
```{r}
p.otu.dist <- distance(p.otu.rel)
nmds <- metaMDS(p.otu.dist)
plot(nmds)

scores(nmds) %>%
  as_tibble(rownames="ID") %>%
  inner_join(., p.map, by="ID") %>%
...
colnames(b.chem.ID)[colnames(b.chem.ID) == "SampleID"] <- "ID"
bs.env <- inner_join(bs.map, b.chem.ID, by="ID")
...
library(ggrepel)
nmds_plot +
  geom_segment(data = as.data.frame(bs.fit$vectors$arrows),
