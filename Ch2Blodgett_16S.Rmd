...
saveRDS(p_map, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/p.map.RDS")
```
...
# pivot table and group by Phylum
tax_bd.long <- pivot_longer(tax_bd, cols=1:45, names_to = "ID", values_to = "counts")
bd.phylum.long <- group_by(tax_bd.long, Phylum, ID) %>%
  dplyr::summarize(counts=sum(counts))
...
# Prep data frame for plotting
bd.phylum.map <- merge(bd.phylum.long, bd_map.pc1, by="ID")
