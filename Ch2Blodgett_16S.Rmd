...
asv.rare <- t(rrarefy(t(asv), sample = 18600))
...
# Plot (I believe this would summarize both depths into each subplot)
s_phylum <- ggplot(s.phylum.map, aes(x=TimeSubPlot, y=counts, fill=factor(NewPhylum, level=collapsed_phyla_order)))+
    labs(x="Sample", 
       y ="Counts",
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        legend.position = "none")
s_phylum
s_phylum <- ggsave("s_phylum.pdf", width = 350, height = 100, units = "mm", dpi = 500)
```
# 3-6 cm (d - deep)
```{r}
# Join taxonomy and abundance data together
...
# Plot (I believe this would summarize both depths into each subplot)
d_phylum <- ggplot(d.phylum.map, aes(x=TimeSubPlot, y=counts, fill=factor(NewPhylum, level=collapsed_phyla_order)))+
    labs(x="Sample", 
       y ="Counts",
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        legend.position = "none")
d_phylum
d_phylum <- ggsave("d_phylum.pdf", width = 350, height = 100, units = "mm", dpi = 500)
```
# control plots (c)
```{r}
# Join taxonomy and abundance data together
