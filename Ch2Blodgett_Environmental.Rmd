...
# U2A
```{r}
temp_U2A=read_delim("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/Field Experiment/Datalogger Temps/U2A.txt", delim = "\t", col_names = TRUE)
...
# B1A + Time above 60ºC
```{r}
temp_B1A=read_delim("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/Field Experiment/Datalogger Temps/B1A.txt", delim = "\t", col_names = TRUE)
...
# B1B + Time above 60ºC
```{r}
temp_B1B=read_delim("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/Field Experiment/Datalogger Temps/B1B.txt", delim = "\t", col_names = TRUE)
...
# B2A + Time above 60ºC
```{r}
temp_B2A=read_delim("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/Field Experiment/Datalogger Temps/B2A.txt", delim = "\t", col_names = TRUE)
...
# B3B + Time above 60ºC
```{r}
temp_B3B=read_delim("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/Field Experiment/Datalogger Temps/B3B.txt", delim = "\t", col_names = TRUE)
...
# B4A
```{r}
temp_B4A=read_delim("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/Field Experiment/Datalogger Temps/U3A.txt", delim = "\t", col_names = TRUE)
...
```{r}
all_temp <- rbind(temp_U1A_long, temp_U2A_long, temp_B1A_long, temp_B1B_long, temp_B2A_long, temp_B3B_long, temp_B4A_long)
all_temp <- all_temp %>%
...
write.csv(all_temp, file = "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/input data/all_temp.csv", row.names = FALSE, na = "", quote = TRUE)
...
ggplot(precip, aes(x = Date, y = Precip_mm, group = 1)) +
  ...
  geom_vline(xintercept = as.numeric(sample_dates), color = "grey40", linetype = "dashed", size = 0.8) +
  geom_vline(xintercept = as.numeric(burn_date), color = "orange", linetype = "dashed", size = 0.8) +
  scale_x_date(
...
# Correlation between viruses (with host prediction) and chemical properties
```{r}
...
loadings$Comp.1 <- -loadings$Comp.1
pca_plot <- ggplot(d_pca_map, aes(x = Comp.1, y = Comp.2, color = Plot)) +
  geom_point(size = 3) + # Plot individual samples
  scale_color_manual(values = treatment_plot_colors, limits = treatment_plot_order) +
  geom_segment(data = loadings,
               aes(x = 0, y = 0, xend = Comp.1 * 10, yend = Comp.2 * 10),  # Scale the vectors for better visibility
...
# Extract PC1 scores
pc1_scores <- as.data.frame(d_pca_fit$scores)[, 1]
d_pc1 <- scores %>% select(1)
d_pc1 <- as.data.frame(d_pc1)
d_pc1 <- rownames_to_column(d_pc1, var = "ID")

saveRDS(d_pc1, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/d_pc1.RDS")
...
pc1_virome <- merge(d_pc1, chem_blodgett[, c("SampleID", "virome")], by = "SampleID", all.x = TRUE)
...
```{r}
library(lme4)
model <- lmer(SoilpH ~ Timepoint * Depth * Disturbance + (1 | Plot/SubPlot), data = b.metadata)
summary(model)

library(emmeans)
emmeans(model, pairwise ~ Timepoint * Depth * Disturbance)
```
...
# STATS!!!
b.metadata$Treatment <- factor(b.metadata$Treatment)
b.metadata$Treatment <- relevel(b.metadata$Treatment, ref = "Control")
intrxn <- lm(Moisture ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)

moisture.stats <- b.metadata
moisture_03_burn <- moisture.stats %>%
  filter(Depth == "0-3cm"&Treatment == "Burn")
kruskal.test(Moisture ~ Timepoint, moisture_03_burn)
wilcox <- pairwise_wilcox_test(
  data = moisture_03_burn,
  formula = Moisture ~ Timepoint,
...
intrxn <- lm(SoilpH ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
```
...
intrxn <- lm(OrganicMatter ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
```
...
intrxn <- lm(InorganicNitrogen ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
```
...
intrxn <- lm(OrganicNInorganicN ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
...
intrxn <- lm(InorganicPhosphorus ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
```
...
intrxn <- lm(Calcium ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
```
# Magnesium
```{r}
...
intrxn <- lm(Magnesium ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)
```
# Potassium
```{r}
...
intrxn <- lm(Potassium ~ Timepoint * Depth * Disturbance, data = b.metadata)
summary(intrxn)

simplified_model <- lm(Potassium ~ Timepoint + Depth + Disturbance, data = b.metadata)
summary(simplified_model)
table(b.metadata$Timepoint, b.metadata$Disturbance)
subset(b.metadata, Timepoint == "T5" & !is.na(Disturbance))


adjusted_model <- lm(Potassium ~ Timepoint + Depth + Disturbance +
                                   Timepoint:Depth +
                                   Depth:Disturbance,
                     data = b.metadata)
...
```
# Subset of Variables
# Burn-Relevant Properties
```{r}
Burn_Properties = b.metadata %>%
  select("SoilpH", "OrganicMatter", "InorganicNitrogen", "OrganicNInorganicN", "InorganicPhosphorus", "OrganicPhosphorus", "Calcium", "Magnesium", "Potassium", "SampleID","Depth","Treatment","Date","Disturbance","Plot", "Timepoint", "virome")
Burn_Properties_long <- melt(Burn_Properties, id.vars=c("SampleID","Depth","Treatment","Date","Disturbance","Plot", "Timepoint", "virome"), measure.vars=colnames(Burn_Properties)[1:9])
...
Burn_Properties_long <- Burn_Properties_long %>%
  mutate(y_axis_label = y_axis_labels[variable])

ggplot(Burn_Properties_long, aes(x = factor(Timepoint, level=c("T1", "T2", "T3", "T4", "T5")), y = value, fill = Treatment)) + 
  annotate('rect', xmin = -Inf, xmax = 2.5, ymin = -Inf, ymax = Inf, fill = 'palegreen', alpha = 0.5) +
  geom_boxplot(outlier.shape=NA) + 
  xlab("Timepoint") + 
  theme_bw() + 
  geom_jitter(aes(color = Depth), position = position_jitter(width = 0.2), size = 1.5) +
  theme(axis.title = element_text(size = 16)) + 
...
ggsave("C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/pdf/BoxPlots/TotalEnvironmental/burnproperties_boxplots.pdf", width = 400, height = 300, units = "mm", dpi = 500)
```
...
# Levene's variation in data???
```{r}
T3.metadata
library(car)
levene_result <- leveneTest(SoilpH ~ Treatment, data = T3.metadata)

levene_function <- function(var) {
  formula <- as.formula(paste(var, "~ Treatment"))
  leveneTest(formula, data = T3.metadata)
}

variable <- variables$Variable

levene_results <- lapply(variable, levene_function)
names(levene_results) <- variable
```
