...
b.chem.ID=b.metadata %>%
  select(-OrganicNRelease, -OrganicPRelease, -SoilHealthCalculation, -OrganicNInorganicN, -WDRFBuffer, -OrganicNReserve, -OrganicPReserve, -AltID, -virome, -Disturbance, -Plot, -SubPlot, -Date, -Treatment, -Burn, -Depth, -Timepoint)
```
...
# All plots (b - blodgett)
```{r}
if(any(is.na(b.nut.mtx))) {
  print("Data contains missing values. Please handle them before PCA.")
} else {
...
loadings(pca_fit)
scree_plot <- data.frame(PC = 1:length(pca_fit$sdev),
...
saveRDS(b.chem.ID, "C:/Users/segeo/Box Sync/EmersonLab/Research/Ch2Blodgett/R_Figures/rds/b.chem.ID.RDS")
...
glm_results <- lapply(chemistry, function(prop) {
  formula <- as.formula(paste(prop, "~ Treatment * Burn"))
...
library(reshape2)
colnames(b.metadata)
b.metadata.long <- melt(b.metadata, id.vars=c("SampleID","Depth","Treatment","Date","Disturbance","Plot"), measure.vars=colnames(b.metadata)[3:38])
```
...
ggsave("blodgett_env_boxplots.pdf", width = 400, height = 400, units = "mm", dpi = 500)
```
