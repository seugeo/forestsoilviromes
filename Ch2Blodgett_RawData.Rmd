...
blodgett.asv <- asv_filtered[,all_map$Experiment=="Heat"]
...
# add environmental cluster data
k_cluster_env_16S <- k_cluster_env %>%
  mutate(ID = gsub("V", "M", ID)) %>%
  mutate(ID = ifelse(ID == "UDM24D", "UDM24", ID))
colnames(k_cluster_env_16S)[colnames(k_cluster_env_16S) == "ID"] <- "MapID"
blodgett_map = left_join(blodgett_map, k_cluster_env_16S, by="MapID")
blodgett_map <- blodgett_map %>%
  mutate(Cluster = as.character(Cluster))
